...
class WebRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path=="/signup":
            data = self.data_string =str(self.rfile.read(int(self.headers['Content-Length'])).decode('UTF-8'))
            ...
            if json_data["id"] not in account_data:
              ...
              data = json.dumps({"success" : False}, indent = 4, sort_keys = True, ensure_ascii = False)
        ...
        # 씨앗구입 완료 // dto : id, itemId
            ...
            account = account_data[json_data['id']]
            # 돈빼기
            account['money'] -= market_data[json_data['itemId']]['price']
        ...
        # 수확하기(씨앗 팔기) // dto : id, farmId, money
            ...
            account_data = json.load(file)
            file.close()
            if  account_data[json_data['id']]['farm'][json_data['farmId']]['item']['complete'] == True:
